...
void set_gemini_volume(uint8_t percent) { if (percent > 100) percent = 100; gemini_volume_percent = percent; ESP_LOGI(TAG, "Volume Gemini diatur ke %u%%", (unsigned)percent); }
uint8_t get_gemini_volume(void) { return gemini_volume_percent; }
...
static void audio_playback_task(void *arg)
{
    ...
    ESP_LOGI(TAG, "Audio playback task: 24kHz PCM16 mono, ring=%u, prebuffer=%u, core=%d priority=4", (unsigned)AUDIO_RING_BUFFER_SIZE, (unsigned)AUDIO_PLAYBACK_PREBUFFER_SIZE, xPortGetCoreID());
    for (;;) {
        if (audio_clear_pending) {
            ...
            audio_turn_complete_pending = false; audio_turn_active = false; playback_started = false; underrun_reported = false; playback_generation = audio_turn_generation;
        }
        uint32_t current_generation = audio_turn_generation;
        if (current_generation != playback_generation) { playback_generation = current_generation; playback_started = false; underrun_reported = false; }
        if (audio_stream == NULL) { vTaskDelay(pdMS_TO_TICKS(100)); continue; }
        size_t pending = xStreamBufferBytesAvailable(audio_stream);
        if (!playback_started && pending < AUDIO_PLAYBACK_PREBUFFER_SIZE && audio_turn_active && !audio_turn_complete_pending) { vTaskDelay(pdMS_TO_TICKS(5)); continue; }
        if (playback_started && pending == 0 && audio_turn_active && !audio_turn_complete_pending) {
            if (!underrun_reported) { ESP_LOGW(TAG, "AUDIO PLAYBACK UNDERRUN: PCM buffer kosong di tengah turn"); underrun_reported = true; }
            vTaskDelay(pdMS_TO_TICKS(AUDIO_PLAYBACK_READ_WAIT_MS)); continue;
        }
        ...
        audio_write_calls++; audio_bytes_played += received; check_audio_playback_complete();
...
bool start_audio_playback(void)
{
    ...
    if (audio_send_mutex == NULL) { audio_send_mutex = xSemaphoreCreateMutexStatic(&audio_send_mutex_storage); if (audio_send_mutex == NULL) { ESP_LOGE(TAG, "Gagal membuat audio send mutex"); return false; } }
    uint8_t *buffer_mem = (uint8_t*)heap_caps_malloc(AUDIO_RING_BUFFER_SIZE, MALLOC_CAP_SPIRAM);
    if (buffer_mem == NULL) { buffer_mem = (uint8_t*)heap_caps_malloc(AUDIO_RING_BUFFER_SIZE, MALLOC_CAP_INTERNAL); if (buffer_mem == NULL) { ESP_LOGE(TAG, "Gagal alokasi %u byte untuk audio buffer", (unsigned)AUDIO_RING_BUFFER_SIZE); return false; } ESP_LOGW(TAG, "Menggunakan RAM internal untuk audio buffer"); }
    ...
    BaseType_t result = xTaskCreatePinnedToCore(audio_playback_task, "audio_playback", 4096, NULL, 4, &audio_playback_task_handle, 1);
    if (result != pdPASS) { ESP_LOGE(TAG, "Gagal membuat audio_task/playback task: free_internal=%u largest=%u", (unsigned)heap_caps_get_free_size(MALLOC_CAP_INTERNAL), (unsigned)heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL)); vStreamBufferDelete(audio_stream); audio_stream = NULL; audio_playback_task_handle = NULL; return false; }
    ESP_LOGI(TAG, "Audio ring buffer siap: %u byte, prebuffer=%u, target=%u B/s, playback core=1 priority=4", (unsigned)AUDIO_RING_BUFFER_SIZE, (unsigned)AUDIO_PLAYBACK_PREBUFFER_SIZE, (unsigned)AUDIO_OUTPUT_BYTES_PER_SEC);
    return true;
}
...
{
    audio_clear_pending = false;
    if (audio_send_mutex != NULL) { xSemaphoreTake(audio_send_mutex, portMAX_DELAY); if (audio_stream != NULL) xStreamBufferReset(audio_stream); xSemaphoreGive(audio_send_mutex); }
    else if (audio_stream != NULL) xStreamBufferReset(audio_stream);
    audio_turn_complete_pending = false; audio_turn_active = false;
}
void reset_audio_turn_stats(void) { audio_chunks_received = 0; audio_bytes_received = 0; audio_bytes_queued = 0; audio_write_calls = 0; audio_bytes_played = 0; audio_bytes_dropped = 0; audio_turn_active = false; audio_turn_complete_pending = false; }
void begin_audio_turn(void)
{
    if (audio_turn_active) return;
    audio_chunks_received = 0; audio_bytes_received = 0; audio_bytes_queued = 0; audio_write_calls = 0; audio_bytes_played = 0; audio_bytes_dropped = 0;
    if (audio_stream != NULL) { size_t stale = xStreamBufferBytesAvailable(audio_stream); if (stale > 0) { xStreamBufferReset(audio_stream); audio_bytes_dropped = stale; ESP_LOGW(TAG, "Audio stale PCM dibuang saat turn baru: %u byte", (unsigned)stale); } }
    uint32_t next_generation = audio_turn_generation + 1U; if (next_generation == 0U) next_generation = 1U; audio_turn_generation = next_generation; audio_turn_active = true; audio_turn_complete_pending = false;
}

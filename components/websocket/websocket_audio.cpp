...
static void audio_playback_task(void *arg)
{
    ...
    ESP_LOGI(TAG, "Audio playback task: 24kHz PCM16 mono, ring=%u, prebuffer=%u, core=%d priority=1", (unsigned)AUDIO_RING_BUFFER_SIZE, (unsigned)AUDIO_PLAYBACK_PREBUFFER_SIZE, xPortGetCoreID());
...
bool start_audio_playback(void)
{
    ...
    BaseType_t result = xTaskCreatePinnedToCore(audio_playback_task, "audio_playback", 4096, NULL, 1, &audio_playback_task_handle, 1);
    if (result != pdPASS) {
        ...
        vStreamBufferDelete(audio_stream); audio_stream = NULL; audio_playback_task_handle = NULL; return false;
    }
    ESP_LOGI(TAG, "Audio ring buffer siap: %u byte, prebuffer=%u, target=%u B/s, playback core=1 priority=1", (unsigned)AUDIO_RING_BUFFER_SIZE, (unsigned)AUDIO_PLAYBACK_PREBUFFER_SIZE, (unsigned)AUDIO_OUTPUT_BYTES_PER_SEC);
    return true;
}
...
{
    audio_chunks_received = 0; audio_bytes_received = 0; audio_bytes_queued = 0; audio_write_calls = 0; audio_bytes_played = 0; audio_bytes_dropped = 0; audio_turn_active = false; audio_turn_complete_pending = false;
}
...
void begin_audio_turn(void)
{
    ...
    if (audio_stream != NULL) {
        ...
        if (stale > 0) { xStreamBufferReset(audio_stream); audio_bytes_dropped = stale; ESP_LOGW(TAG, "Audio stale PCM dibuang saat turn baru: %u byte", (unsigned)stale); }
    }
...
bool queue_audio_pcm(const uint8_t *pcm, size_t len)
{
    ...
    if (audio_send_mutex == NULL) { ESP_LOGE(TAG, "Audio send mutex belum siap"); return false; }
    ...
    if (gemini_volume_percent < 100) {
        ...
        while (offset < total_samples) {
            ...
            if (chunk > sizeof(volume_buffer) / sizeof(volume_buffer[0])) chunk = sizeof(volume_buffer) / sizeof(volume_buffer[0]);
            for (size_t i = 0; i < chunk; i++) volume_buffer[i] = (int16_t)(src[offset + i] * gemini_volume_percent / 100);
